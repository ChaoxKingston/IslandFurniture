    ...
    <body>
        <%
            ...
            for (int a=0;a<furnitures.size();a++) {
                if (furnitures.get(a).getSKU().equals(sku)) {
                    fn = furnitures.get(a);
                }
            }
        %>
                    ...
                    <div class="container">
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div>
                                    <div class="thumbnail">
                                        <img alt="" class="img-responsive img-rounded" src="../../..<%=fn.getImageUrl()%>">
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="summary entry-summary">
                                    <h2 class="shorter"><strong><%=fn.getCategory()%></strong></h2>
                                    <%
                                        if (isMemberLoggedIn == true) {
                                    %>
                                    <form action="../../ECommerce_AddFurnitureToListServlet">
                                        <input type="hidden" name="id" value="<%=fn.getId()%>"/>
                                        <input type="hidden" name="SKU" value="<%=fn.getSKU()%>"/>
                                        <input type="hidden" name="price" value="<%=fn.getPrice()%>"/>
                                        <input type="hidden" name="name" value="<%=fn.getName()%>"/>
                                        <input type="hidden" name="imageURL" value="<%=fn.getImageUrl()%>"/>
                                        <input type="submit" name="btnEdit" class="btn btn-primary" id="<%=fn.getSKU()%>" value="Add To Cart"/>
                                    </form>
                                    <%}%>
                                    <p class="price"><h4 class="amount"><%=fn.getPrice()%></h4></p>
                                    <strong>Description</strong>
                                    <p class="taller">
                                        <%=fn.getDescription()%>
                                    </p>
                                    <p>
                                        Height: <%=fn.getHeight()%><br/>
                                        Length: <%=fn.getLength()%><br/>
                                        Width: <%=fn.getWidth()%>
                                    </p>
                                    <div class="product_meta">
                                            <span class="posted_in">Category: <a rel="tag" href="../../ECommerce_FurnitureCategoryServlet?cat=<%=URLEncoder.encode(fn.getCategory(), "UTF-8")%>"><%=fn.getCategory()%></a></span>
                                    </div>
                                    <br/><br/>
